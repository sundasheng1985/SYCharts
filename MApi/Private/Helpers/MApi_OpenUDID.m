...
//  Copyright 2011, 2012 MApi_OpenUDID.org
//
//  Initiators/root branches
//      iOS code: https://github.com/ylechelle/MApi_OpenUDID
//      Android code: https://github.com/vieux/MApi_OpenUDID
//
//  Contributors:
//      https://github.com/ylechelle/MApi_OpenUDID/contributors
//
...
#define MApi_OpenUDIDLog(fmt, ...)
//#define MApi_OpenUDIDLog(fmt, ...) NSLog((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define MApi_OpenUDIDLog(fmt, ...) NSLog((@"[Line %d] " fmt), __LINE__, ##__VA_ARGS__);

static NSString * kOpenUDIDSessionCache = nil;
//static NSString * const kOpenUDIDDescription = @"MApi_OpenUDID_with_iOS6_Support";
static NSString * const kOpenUDIDKey = @"MApi_OpenUDID";
static NSString * const kOpenUDIDSlotKey = @"MApi_OpenUDID_slot";
static NSString * const kOpenUDIDAppUIDKey = @"MApi_OpenUDID_appUID";
static NSString * const kOpenUDIDTSKey = @"MApi_OpenUDID_createdTS";
static NSString * const kOpenUDIDOOTSKey = @"MApi_OpenUDID_optOutTS";
static NSString * const kOpenUDIDDomain = @"org.MApi_OpenUDID";
static NSString * const kOpenUDIDSlotPBPrefix = @"org.MApi_OpenUDID.slot.";
static int const kOpenUDIDRedundancySlots = 100;

@interface MApi_OpenUDID (Private)
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
    ...
    if (item) {
        @try{
            ...
            MApi_OpenUDIDLog(@"Unable to unarchive item %@ on pasteboard!", [pboard name]);
            item = nil;
        }
    }
...
// Private method to create and return a new MApi_OpenUDID
// Theoretically, this function is called once ever per application when calling [MApi_OpenUDID value] for the first time.
// After that, the caching/pasteboard/redundancy mechanism inside [MApi_OpenUDID value] returns a persistent and cross application MApi_OpenUDID
//
+ (NSString*) _generateFreshOpenUDID {
...
//    }
    ...
    // compile time. Accredited developers integrating MApi_OpenUDID are expected to review the iOS 6 release notes and
    ...
    //
    // Essentially, this means that MApi_OpenUDID will keep on behaving the same way as before for existing users or
    // new users in iOS 5 and before. For new users on iOS 6 and after, the new official public APIs will take over.
    // MApi_OpenUDID will therefore be obsoleted when iOS reaches significant adoption, anticipated around mid-2013.
...
// Main public method that returns the MApi_OpenUDID
// This method will generate and store the MApi_OpenUDID if it doesn't exist, typically the first time it is called
...
// Otherwise, it will register the current app and return the MApi_OpenUDID
//
+ (NSString*) value {
    return [MApi_OpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if (kOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:kOpenUDIDDomain
                                     ...
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"MApi_OpenUDID in cache from first call",@"description", nil]];
    ...
    // Do we have a local copy of the MApi_OpenUDID dictionary?
    ...
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        MApi_OpenUDIDLog(@"localDict = %@",localDict);
    }
    
    // Here we go through a sequence of slots, each of which being a UIPasteboard created by each participating app
    // The idea behind this is to both multiple and redundant representations of MApi_OpenUDIDs, as well as serve as placeholder for potential opt-out
    //
    ...
    // sort the Frequency dict with highest occurence count of the same MApi_OpenUDID (redundancy, failsafe)
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            // this is the case where this app instance is likely to be the first one to use MApi_OpenUDID on this device
            // we create the MApi_OpenUDID, legacy or semi-random (i.e. most certainly unique)
            //
            openUDID = [MApi_OpenUDID _generateFreshOpenUDID];
        } else {
            // or we leverage the MApi_OpenUDID shared by other apps that have already gone through the process
            // 
    ...
    //
    MApi_OpenUDIDLog(@"Available Slot %@ Existing Slot %@",availableSlotPBid,myRedundancySlotPBid);
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [MApi_OpenUDID _setDict:localDict forPasteboard:slotPB];
    }
    ...
    // If the UIPasteboard external representation marks this app as opted-out, then to respect privacy, we return the ZERO MApi_OpenUDID, a sequence of 40 zeros...
    ...
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:kOpenUDIDDomain
                                                 ...
                                                 userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Application with unique id %@ is opted-out from MApi_OpenUDID as of %@",appUID,optedOutDate],@"description", nil]];
            
        kOpenUDIDSessionCache = [[NSString stringWithFormat:@"%040x",0] copy];
        return kOpenUDIDSessionCache;
    }
    ...
    if (error!=nil) {
        if (isCompromised)
            *error = [NSError errorWithDomain:kOpenUDIDDomain
                                     ...
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Found a discrepancy between stored MApi_OpenUDID (reliable) and redundant copies; one of the apps on the device is most likely corrupting the MApi_OpenUDID protocol",@"description", nil]];
        else
            *error = [NSError errorWithDomain:kOpenUDIDDomain
                                         code:kOpenUDIDErrorNone
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"MApi_OpenUDID succesfully retrieved",@"description", nil]];
    }
    kOpenUDIDSessionCache = [openUDID copy];
    return kOpenUDIDSessionCache;
}
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    MApi_OpenUDIDLog(@"Local dict after opt-out = %@",dict);
