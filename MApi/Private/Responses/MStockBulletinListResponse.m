...
#import "MStockBulletinListResponse.h"

@implementation MStockBulletinListResponse
    ...
    if (self) {
        ...
        if ([self getJSONObject:&JSONValues withData:data parseClass:NSDictionary.class]) {
            ...
            for (NSDictionary *newsJSONValue in JSONValues[@"List"]) {
                id item = [MStockBulletinItem itemWithJSONObject:newsJSONValue];
                if (item) {
