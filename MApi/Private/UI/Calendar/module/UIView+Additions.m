...
//  MitakeCore
//
//  Created by mitake-cn on 2015/2/9.
...
- (void)setBorderWidth:(CGFloat)width withColor:(UIColor *)color direction:(UIViewCustomBorderDirection)direction {
    [self setBorderWidth:width withColor:color edgeInsets:UIEdgeInsetsZero direction:direction];
}

- (void)setBorderWidth:(CGFloat)width withColor:(UIColor *)color edgeInsets:(UIEdgeInsets)insets direction:(UIViewCustomBorderDirection)direction {
    width = width / [[UIScreen mainScreen] scale];
    //    self.layer.sublayers = nil;
    
    if ((direction & UIViewCustomBorderDirectionTop) == UIViewCustomBorderDirectionTop) {
        
        CALayer *sublayer = [self sublayerWithName:@"top_layer"];
        if (sublayer == nil) {
            sublayer = [CALayer layer];
            sublayer.name = @"top_layer";
            [self.layer addSublayer:sublayer];
        }
        sublayer.frame = CGRectMake(0 + insets.left,
                                    0 + insets.top - insets.bottom,
                                    CGRectGetWidth(self.bounds) - (insets.left + insets.right), width);
        sublayer.backgroundColor = color.CGColor;
    }
    if ((direction & UIViewCustomBorderDirectionLeft) == UIViewCustomBorderDirectionLeft) {
        CALayer *sublayer = [self sublayerWithName:@"left_layer"];
        if (sublayer == nil) {
            sublayer = [CALayer layer];
            sublayer.name = @"left_layer";
            [self.layer addSublayer:sublayer];
        }
        sublayer.frame = CGRectMake(0 + insets.left - insets.right,
                                    0 + insets.top,
                                    width,
                                    CGRectGetHeight(self.bounds) - (insets.top + insets.bottom));
        sublayer.backgroundColor = color.CGColor;
    }
    if ((direction & UIViewCustomBorderDirectionBottom) == UIViewCustomBorderDirectionBottom) {
        CALayer *sublayer = [self sublayerWithName:@"bottom_layer"];
        if (sublayer == nil) {
            sublayer = [CALayer layer];
            sublayer.name = @"bottom_layer";
            [self.layer addSublayer:sublayer];
        }
        sublayer.frame = CGRectMake(0 + insets.left,
                                    CGRectGetHeight(self.bounds)-width + insets.top - insets.bottom,
                                    CGRectGetWidth(self.bounds) - (insets.left + insets.right),
                                    width);
        sublayer.backgroundColor = color.CGColor;
    }
    if ((direction & UIViewCustomBorderDirectionRight) == UIViewCustomBorderDirectionRight) {
        CALayer *sublayer = [self sublayerWithName:@"right_layer"];
        if (sublayer == nil) {
            sublayer = [CALayer layer];
            sublayer.name = @"right_layer";
            [self.layer addSublayer:sublayer];
        }
        sublayer.frame = CGRectMake(CGRectGetWidth(self.bounds)-width + insets.left - insets.right,
                                    0 + insets.top,
                                    width,
                                    CGRectGetHeight(self.bounds) - (insets.top + insets.bottom));
        sublayer.backgroundColor = color.CGColor;
