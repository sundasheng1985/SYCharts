...
@interface MApi_MQTTSession(Create)
 ...
 */
- (MApi_MQTTSession *)initWithClientId:(NSString *)clientId
...
/** initialises the MApi_MQTT session
 *
 ...
 * and other vulnerabilities. you may need to set securityPolicy to properly value(see MApi_MQTTSSLSecurityPolicy.h for more detail).
 *
 ...
 * MApi_MQTTSSLSecurityPolicy.
 * And if you use self-signed server certificates, your need to set property: MApi_MQTTSSLSecurityPolicy.allowInvalidCertificates=YES
 ...
 * @param keepAliveInterval The Keep Alive is a time interval measured in seconds. The MApi_MQTTClient ensures that the interval between Control Packets being sent does not exceed the Keep Alive value. In the  absence of sending any other Control Packets, the Client sends a PINGREQ Packet.
 ...
 * @code
 ...
 NSString* certificate = [[NSBundle bundleForClass:[MApi_MQTTSession class]] pathForResource:@"certificate" ofType:@"cer"];
 MApi_MQTTSSLSecurityPolicy *securityPolicy = [MApi_MQTTSSLSecurityPolicy policyWithPinningMode:MApi_MQTTSSLPinningModeCertificate];
 ...
 MApi_MQTTSession *session = [[MApi_MQTTSession alloc]
...
/** for mqttio-OBJC backward compatibility
 @param theClientId see initWithClientId for description.
 @param theKeepAliveInterval see initWithClientId for description.
 @param theConnectMessage has to be constructed using MApi_MQTTMessage connectMessage...
 @param theRunLoop see initWithClientId for description.
 @param theRunLoopMode see initWithClientId for description.
 @return the initialised MApi_MQTTSession object
 All other parameters are set to defaults
...
- (id)initWithClientId:(NSString*)theClientId
             keepAlive:(UInt16)theKeepAliveInterval
        connectMessage:(MApi_MQTTMessage*)theConnectMessage
               runLoop:(NSRunLoop*)theRunLoop
               forMode:(NSString*)theRunLoopMode;
...
- (void)connectToHost:(NSString *)host
       ...
       connectHandler:(MApi_MQTTConnectHandler)connectHandler;
...
- (void)connectToHost:(NSString*)ip
                 port:(UInt32)port
withConnectionHandler:(void (^)(MApi_MQTTSessionEvent event))connHandler
