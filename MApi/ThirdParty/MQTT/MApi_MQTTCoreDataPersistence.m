//
//  MApi_MQTTCoreDataPersistence.m
//  MApi_MQTTClient
//
...
@interface MApi_MQTTCoreDataFlow ()

- (MApi_MQTTCoreDataFlow *)initWithContext:(NSManagedObjectContext *)context andObject:(id<MApi_MQTTFlow>)object;
@property NSManagedObjectContext *context;
@property id<MApi_MQTTFlow> object;

@end

@implementation MApi_MQTTCoreDataFlow

@synthesize context;
@synthesize object;

- (MApi_MQTTCoreDataFlow *)initWithContext:(NSManagedObjectContext *)c andObject:(id<MApi_MQTTFlow>)o
{
    self = [super init];
    self.context = c;
    self.object = o;
...
@implementation MApi_MQTTCoreDataPersistence
@synthesize persistent;
@synthesize maxSize;
@synthesize maxMessages;
@synthesize maxWindowSize;

- (MApi_MQTTCoreDataPersistence *)init {
    self = [super init];
...
- (NSUInteger)windowSize:(NSString *)clientId {
    ...
    NSArray *flows = [self allFlowsforClientId:clientId
                                  incomingFlag:NO];
    for (MApi_MQTTCoreDataFlow *flow in flows) {
        if ([flow.commandType unsignedIntegerValue] != MApi_MQTT_None) {
            windowSize++;
        }
    }
    return windowSize;
...
- (MApi_MQTTCoreDataFlow *)storeMessageForClientId:(NSString *)clientId
    ...
    if (([self allFlowsforClientId:clientId incomingFlag:incomingFlag].count <= self.maxMessages) &&
        ...
        MApi_MQTTCoreDataFlow *flow = [self createFlowforClientId:clientId
...
- (void)deleteFlow:(MApi_MQTTCoreDataFlow *)flow {
    if ([NSThread isMainThread]) {
...
- (void)deleteAllFlowsForClientId:(NSString *)clientId {
    if ([NSThread isMainThread]) {
        for (MApi_MQTTCoreDataFlow *flow in [self allFlowsforClientId:clientId incomingFlag:TRUE]) {
            [self.managedObjectContext deleteObject:(NSManagedObject *)flow.object];
        }
        for (MApi_MQTTCoreDataFlow *flow in [self allFlowsforClientId:clientId incomingFlag:FALSE]) {
            [self.managedObjectContext deleteObject:(NSManagedObject *)flow.object];
        }
    } else {
        [self.managedObjectContext performBlockAndWait:^{
...
- (void)internalSync {
    if (self.managedObjectContext.hasChanges) {
        DDLogVerbose(@"[MApi_MQTTPersistence] pre-sync: i%lu u%lu d%lu",
        ...
        if (![self.managedObjectContext save:&error]) {
            DDLogError(@"[MApi_MQTTPersistence] sync error %@", error);
        }
        if (self.managedObjectContext.hasChanges) {
            DDLogError(@"[MApi_MQTTPersistence] sync not complete");
        }
        DDLogVerbose(@"[MApi_MQTTPersistence] postsync: i%lu u%lu d%lu",
                     (unsigned long)self.managedObjectContext.insertedObjects.count,
                     (unsigned long)self.managedObjectContext.updatedObjects.count,
                     (unsigned long)self.managedObjectContext.deletedObjects.count
                     );
...
- (void)internalParentSync {
    if (self.managedObjectContext.parentContext && self.managedObjectContext.parentContext.hasChanges) {
        ...
        if (![self.managedObjectContext.parentContext save:&error]) {
            DDLogError(@"[MApi_MQTTPersistence] parentContext sync error %@", error);
        }
    }
}
...
- (NSArray *)allFlowsforClientId:(NSString *)clientId
    ...
    if ([NSThread isMainThread]) {
        NSFetchRequest *fetchRequest = [NSFetchRequest fetchRequestWithEntityName:@"MApi_MQTTFlow"];
        ...
        if (!rows) {
            DDLogError(@"[MApi_MQTTPersistence] allFlowsforClientId %@", error);
        }
    } else {
        [self.managedObjectContext performBlockAndWait:^{
    ...
    for (id<MApi_MQTTFlow>row in rows) {
        [flows addObject:[[MApi_MQTTCoreDataFlow alloc] initWithContext:self.managedObjectContext andObject:row]];
    }
    return flows;
}

- (MApi_MQTTCoreDataFlow *)flowforClientId:(NSString *)clientId
                         incomingFlag:(BOOL)incomingFlag
                            messageId:(UInt16)messageId {
    __block MApi_MQTTCoreDataFlow *flow = nil;
...
- (MApi_MQTTCoreDataFlow *)internalFlowForClientId:(NSString *)clientId
                                 incomingFlag:(BOOL)incomingFlag
                                    messageId:(UInt16)messageId {
    MApi_MQTTCoreDataFlow *flow = nil;
    ...
    rows = [self.managedObjectContext executeFetchRequest:fetchRequest error:&error];
    if (!rows) {
        DDLogError(@"[MApi_MQTTPersistence] flowForClientId %@", error);
    } else {
        if ([rows count]) {
            flow = [[MApi_MQTTCoreDataFlow alloc] initWithContext:self.managedObjectContext andObject:[rows lastObject]];
        }
    }
    return flow;
}

- (MApi_MQTTCoreDataFlow *)createFlowforClientId:(NSString *)clientId
                               incomingFlag:(BOOL)incomingFlag
                                  messageId:(UInt16)messageId {
    MApi_MQTTCoreDataFlow *flow = (MApi_MQTTCoreDataFlow *)[self flowforClientId:clientId
                                                          incomingFlag:incomingFlag
                                                             messageId:messageId];
    if (!flow) {
        __block id<MApi_MQTTFlow> row;
        if ([NSThread isMainThread]) {
            row = [NSEntityDescription insertNewObjectForEntityForName:@"MApi_MQTTFlow"
        ...
        flow = [[MApi_MQTTCoreDataFlow alloc] initWithContext:self.managedObjectContext andObject:row];
    }
...
{
    NSManagedObjectContext *managedObjectContext = [[NSThread currentThread].threadDictionary valueForKey:@"MApi_MQTTClient"];
    ...
    @synchronized (lock) {
        ...
        [[NSThread currentThread].threadDictionary setObject:managedObjectContext forKey:@"MApi_MQTTClient"];
...
- (NSManagedObjectModel *)managedObjectModel
{
    @synchronized (lock) {
        ...
        entityDescription.name = @"MApi_MQTTFlow";
        entityDescription.managedObjectClassName = @"MApi_MQTTFlow";
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    @synchronized (lock) {
        ...
        NSURL *persistentStoreURL = [[self applicationDocumentsDirectory]
                                     URLByAppendingPathComponent:@"MApi_MQTTClient"];
        DDLogInfo(@"[MApi_MQTTPersistence] Persistent store: %@", persistentStoreURL.path);
        ...
        if (![persistentStoreCoordinator addPersistentStoreWithType:self.persistent ? NSSQLiteStoreType : NSInMemoryStoreType
                                                      configuration:nil
                                                            ...
                                                            options:options
                                                              error:&error]) {
            DDLogError(@"[MApi_MQTTPersistence] managedObjectContext save: %@", error);
...
- (void)sizes {
    if (self.persistent) {
        ...
        NSString *persistentStorePath = [documentsDirectory stringByAppendingPathComponent:@"MApi_MQTTClient"];
    ...
    DDLogVerbose(@"[MApi_MQTTPersistence] sizes %llu/%llu", fileSize, fileSystemFreeSize);
}
@end
