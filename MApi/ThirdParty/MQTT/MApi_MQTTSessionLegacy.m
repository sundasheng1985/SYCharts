...
@implementation MApi_MQTTSession(Legacy)
...
- (MApi_MQTTSession *)initWithClientId:(NSString *)clientId
    ...
    DDLogVerbose(@"[MApi_MQTTSessionLegacy] initWithClientId:%@ ", clientId);
...
- (id)initWithClientId:(NSString*)theClientId {
    ...
    return [self initWithClientId:theClientId
                     ...
                     cleanSession:YES
                        ...
                        willTopic:nil
                          willMsg:nil
                          willQoS:MApi_MQTTQosLevelAtMostOnce
...
- (void)connectToHost:(NSString *)host
    ...
    DDLogVerbose(@"MApi_MQTTSessionLegacy connectToHost:%@ port:%d usingSSL:%d connectHandler:%p",
                 host, (unsigned int)port, usingSSL, connectHandler);
    
    if (self.securityPolicy) {
        MApi_MQTTSSLSecurityPolicyTransport *transport = [[MApi_MQTTSSLSecurityPolicyTransport alloc] init];
        transport.host = host;
        transport.port = port;
...
- (void)subscribeTopic:(NSString*)theTopic {
    [self subscribeToTopic:theTopic atLevel:MApi_MQTTQosLevelAtLeastOnce];
}

- (void)publishData:(NSData*)theData onTopic:(NSString*)theTopic {
    [self publishData:theData onTopic:theTopic retain:NO qos:MApi_MQTTQosLevelAtLeastOnce];
}
...
- (void)publishDataAtLeastOnce:(NSData*)theData onTopic:(NSString*)theTopic retain:(BOOL)retainFlag {
    [self publishData:theData onTopic:theTopic retain:retainFlag qos:MApi_MQTTQosLevelAtLeastOnce];
}

- (void)publishDataAtMostOnce:(NSData*)theData onTopic:(NSString*)theTopic {
    [self publishData:theData onTopic:theTopic retain:NO qos:MApi_MQTTQosLevelAtMostOnce];
}

- (void)publishDataAtMostOnce:(NSData*)theData onTopic:(NSString*)theTopic retain:(BOOL)retainFlag {
    [self publishData:theData onTopic:theTopic retain:retainFlag qos:MApi_MQTTQosLevelAtMostOnce];
}

- (void)publishDataExactlyOnce:(NSData*)theData onTopic:(NSString*)theTopic {
    [self publishData:theData onTopic:theTopic retain:NO qos:MApi_MQTTQosLevelExactlyOnce];
}

- (void)publishDataExactlyOnce:(NSData*)theData onTopic:(NSString*)theTopic retain:(BOOL)retainFlag {
    [self publishData:theData onTopic:theTopic retain:retainFlag qos:MApi_MQTTQosLevelExactlyOnce];
}

- (void)publishJson:(id)payload onTopic:(NSString*)theTopic {
    NSData *data = [NSJSONSerialization dataWithJSONObject:payload options:0 error:nil];
    if (data) {
        [self publishData:data onTopic:theTopic retain:FALSE qos:MApi_MQTTQosLevelAtLeastOnce];
    }
}
@end
