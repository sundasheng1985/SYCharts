 ...
 */
@protocol MApi_MQTTSessionManagerDelegate <NSObject>

/**
 Enumeration of MApi_MQTTSessionManagerState values
 */
typedef NS_ENUM(int, MApi_MQTTSessionManagerState) {
    MApi_MQTTSessionManagerStateStarting,
    MApi_MQTTSessionManagerStateConnecting,
    MApi_MQTTSessionManagerStateError,
    MApi_MQTTSessionManagerStateConnected,
    MApi_MQTTSessionManagerStateClosing,
    MApi_MQTTSessionManagerStateClosed
};
...
/** SessionManager handles the MApi_MQTT session for your application
 */
@interface MApi_MQTTSessionManager : NSObject <MApi_MQTTSessionDelegate>

/** the delegate receiving incoming messages
 */
@property (weak, nonatomic) id<MApi_MQTTSessionManagerDelegate> delegate;

/** subscriptions is a dictionary of NSNumber instances indicating the MApi_MQTTQoSLevel.
...
/** effectiveSubscriptions s a dictionary of NSNumber instances indicating the granted MApi_MQTTQoSLevel, or 0x80 for subscription failure.
 *  The keys are topic filters.
 *  effectiveSubscriptions is observable and is updated everytime subscriptions change
 *  @code
        ...
        MApi_MQTTSessionManager *manager = [[MApi_MQTTSessionManager alloc] init];
    ...
    - (void)observeValueForKeyPath:(NSString *)keyPath
        ...
        if ([keyPath isEqualToString:@"effectiveSubscriptions"]) {
            MApi_MQTTSessionManager *manager = (MApi_MQTTSessionManager *)object;
 ...
 */
@property (nonatomic, readonly) MApi_MQTTSessionManagerState state;

/** SessionManager last error code when state equals MApi_MQTTSessionManagerStateError
 */
@property (nonatomic, readonly) NSError *lastErrorCode;

/** initWithPersistence sets the MApi_MQTTPersistence properties other than default
 ...
 * @return the initialized MApi_MQTTSessionManager object
 */
...
/** Connects to the MApi_MQTT broker and stores the parameters for subsequent reconnects
 ...
 * @param keepalive The Keep Alive is a time interval measured in seconds. The MApi_MQTTClient ensures that the interval between Control Packets being sent does not exceed the Keep Alive value. In the  absence of sending any other Control Packets, the Client sends a PINGREQ Packet.
...
/** Re-Connects to the MApi_MQTT broker using the parameters for given in the connectTo method
 */
- (void)connectToLast;
...
/** Disconnects gracefully from the MApi_MQTT broker
 */
- (void)disconnect;
