...
@implementation NSData (MApi_GZIP)

- (NSData *)mapi_gzippedDataWithCompressionLevel:(float)level
{
    if ([self length])
    {
        z_stream stream;
...
- (NSData *)mapi_gzippedData
{
    return [self mapi_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)mapi_gunzippedData
{
    if ([self length])
    {
        z_stream stream;
