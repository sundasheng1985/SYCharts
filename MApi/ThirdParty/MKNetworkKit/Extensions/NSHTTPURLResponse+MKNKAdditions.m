...
@implementation NSHTTPURLResponse (MApi_MKNKAdditions)

-(BOOL) isContentTypeImage {
  
  NSString *contentType = [self.allHeaderFields mapi_objectForCaseInsensitiveKey:@"Content-Type"];
  return ([contentType.lowercaseString rangeOfString:@"image"].location != NSNotFound);
}

-(BOOL) hasDoNotCacheDirective {
  
  NSString *cacheControl = [self.allHeaderFields mapi_objectForCaseInsensitiveKey:@"Cache-Control"];
...
-(BOOL) hasHTTPCacheHeaders {
  ...
  NSString *cacheControl = [self.allHeaderFields mapi_objectForCaseInsensitiveKey:@"Cache-Control"];
  NSString *eTag = [self.allHeaderFields mapi_objectForCaseInsensitiveKey:@"ETag"];
  NSString *lastModified = [self.allHeaderFields mapi_objectForCaseInsensitiveKey:@"Last-Modified"];
...
-(NSDate*) cacheExpiryDate {
  ...
  NSString *expiresOn = [self.allHeaderFields mapi_objectForCaseInsensitiveKey:@"Expires"];
  __block NSDate *expiresOnDate = [NSDate mapi_dateFromRFC1123:expiresOn];
